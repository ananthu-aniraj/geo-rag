...
* **Automated Decision Heuristic**:
  ...
  1. **Dynamic Drift Analysis (`check_semantic_drift.py`)**: If a clustered parquet database file already exists on disk for the current $k$, the pipeline extracts embeddings from the new images in the dataset. It queries them against the old centroids using FAISS and measures their cosine similarities.

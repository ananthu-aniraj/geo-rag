...
- **Environment Token Security Overhaul**: Secured the entire pipeline from hardcoded API tokens. Modified `run_full_pipeline.sh` to automatically source environment variables from a gitignored `.env` file and forwarded `HF_TOKEN` dynamically to the SGLang Docker container. Removed all hardcoded fallback Mapillary API tokens across all Python pipeline modules (`label_clusters_mllm.py`, `relabel_failed_clusters.py`, `visualize_clusters.py`, `visualize_cluster_samples.py`, `backfill_embeddings.py`, and `process_scraped_data.py`), replacing them with dynamic OS environment variable lookups and local `.env` file parsing fallbacks.

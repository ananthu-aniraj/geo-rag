...
| **`geo_space_{representation_type}_embeddings.npy`** | NumPy Binary Matrix | ~7.9 GB / ~15.9 GB | Decoupled parent feature representations matrix containing embeddings. E.g. `geo_space_cls_embeddings.npy` (D=768) containing parent dataset embeddings. |
| **`geo_space_cleaned_{representation_type}_embeddings.npy`** | NumPy Binary Matrix | ~7.1 GB / ~14.3 GB | Decoupled feature representations matrix containing embeddings aligned to the coordinate-cleaned subset (free of anomalies). |
...
```mermaid
graph TD
    A["Base Metadata<br>geo_space_cleaned.parquet<br>(Photo_ID, Platform, Lat, Lon)"] <-->|1-to-1 Row Index| B["Cleaned Embedding Matrix<br>geo_space_cleaned_cls_embeddings.npy<br>(N_cleaned, 768)"]
...
1. **1-to-1 Row Alignment:**
   The $i$-th row of the NumPy matrix in `geo_space_cleaned_cls_embeddings.npy` corresponds *exactly* to the $i$-th row of the metadata in `geo_space_cleaned.parquet`. Slicing operations can be performed directly on disk via memory-mapping (`mmap_mode="r"`).
   When coordinate anomalies are pruned, the companion `.npy` file is filtered to produce a dedicated `_cleaned_` `.npy` file and the `embedding_idx` column is reset to sequential dense values (`0, 1, 2, ..., N-1`) to preserve 1-to-1 alignment.
2. **Sidecar Merging:**
...
The embeddings loader (`load_embeddings`) automatically resolves companion `.npy` paths. It looks for a dedicated file matching the parquet base name first (e.g. loading `geo_space_cleaned_cls_embeddings.npy` for `geo_space_cleaned.parquet`). If not present, it dynamically falls back to the parent uncleaned file (e.g., `geo_space_cls_embeddings.npy` for backward compatibility):
```python
...
print(embeddings.shape) # (N_cleaned, 768)

...
| **`geo_space_{representation_type}_embeddings.npy`** | NumPy Binary Matrix | ~7.9 GB / ~15.9 GB | Decoupled feature representations matrix of shape `(N, D)` containing embeddings. E.g. `_cls_embeddings.npy` (D=768), `_avg_patch_embeddings.npy` (D=768), or `_cls_avg_patch_embeddings.npy` (D=1536). Can be saved in `float32` or `float16` precision (saving 50% disk storage). |
...
The embeddings loader dynamically resolves slices using the parquet file's index mappings. It automatically handles glob fallback lookups for suffixes and upcasts float16 stored values to float32 in RAM:
```python
...
embeddings = load_embeddings(
    "full_pipeline_output/geo_space_clustered_k_40000.parquet",
    representation_type="cls_avg_patch" # Omit to fallback to wildcard glob matching
)

print(embeddings.shape) # (N, 1536)

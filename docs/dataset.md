...
| **`geo_space_{representation_type}_embeddings.keys.parquet`** | Apache Parquet | ~45 MB | Companion keys index mapping each row of the `.npy` matrix to its stable `photo_key` string (`{platform}_{photo_id}`). |

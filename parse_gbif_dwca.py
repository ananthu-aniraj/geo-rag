...
def build_media_mapping(zip_file, multimedia_file):
    ...
    with zip_file.open(multimedia_file) as f:
        ...
        for chunk in chunks:
            ...
            chunk = chunk[chunk['type'].str.lower() == 'stillimage'].dropna(subset=['gbifID', 'identifier'])
            ...
            idx += 1
            print(f"  Processed {idx * 500000:,} multimedia rows... (Mapped {len(media_map):,} unique observation images)")
            
    print(f"Mapping built in {time.time() - start_time:.2f} seconds. Total observations mapped: {len(media_map):,}")
    return media_map
...
def process_dwca_zip(zip_path, output_dir, chunk_size=100000, max_records=None):
    ...
    with zipfile.ZipFile(zip_path) as z:
        namelist = z.namelist()
        multimedia_file = next((f for f in namelist if f.endswith('multimedia.txt')), None)
        occurrence_file = next((f for f in namelist if f.endswith('occurrence.txt')), None)
        
        if not multimedia_file or not occurrence_file:
            print("Error: Could not find 'multimedia.txt' or 'occurrence.txt' inside the DwC-A ZIP.")
        ...
        with z.open(occurrence_file) as f:
            ...
            for df_chunk in chunks:
                ...
                df_chunk = df_chunk.dropna(subset=['decimalLatitude', 'decimalLongitude'])
    ...
    print("\n" + "="*50)
    print(f"DwC-A Parsing Complete!")
    print(f"Processed: {total_processed} observations in {time.time() - start_time:.2f} seconds.")
    print(f"Output files saved to: {output_dir}")
    print("="*50)

def main():
    parser = argparse.ArgumentParser(description="Parse GBIF Darwin Core Archive (DwC-A) ZIP file directly.")

...
def process_gbif_zip(zip_path, output_dir, chunk_size=100000, max_records=None):
    ...
    with zipfile.ZipFile(zip_path) as z:
        ...
        occurrence_file = next(
            (f for f in namelist if f.endswith(".txt") or f.endswith(".csv")), None
        )

        if not occurrence_file:
            print(
                "Error: Could not find any occurrence data file (.txt or .csv) inside the ZIP archive."
            )
        ...
        with z.open(occurrence_file) as f:
            ...
            for df_chunk in chunks:
                ...
                df_chunk = df_chunk.dropna(
                    subset=["decimalLatitude", "decimalLongitude", "associatedMedia"]
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Parse GBIF iNaturalist Occurrence ZIP file directly."
    )
    parser.add_argument(
        "--input", type=str, required=True, help="Path to the downloaded GBIF ZIP file."
    )
    parser.add_argument(
        "--out_dir",
        type=str,

...
def main():
    parser = argparse.ArgumentParser(
        description="Generate an interactive H3 spatial-semantic world map."
    )
    parser.add_argument(
        "--index",
        type=str,
        default="full_pipeline_output/geo_space_h3_semantic_index.parquet",
        help="Path to the pre-aggregated H3 semantic index Parquet file.",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="full_pipeline_output/global_h3_semantic_map.html",
    ...
    global_cell_totals = global_cell_totals[
        global_cell_totals["image_count"] >= args.min_count
    ]

    if global_cell_totals.empty:
        print(
            f"No H3 cells found with at least {args.min_count} images at resolution {args.res}."
        )
        return
    ...
    unique_seasons = (
        sorted(df_res["Season"].unique()) if "Season" in df_res.columns else ["Unknown"]
    )
    ...
    for season in unique_seasons:
        df_season = (
            df_res[df_res["Season"] == season] if "Season" in df_res.columns else df_res
        )
        ...
        df_cell_agg = (
            df_season.groupby(agg_cols, observed=True)["image_count"]
            .sum()
            .reset_index()
        )
        df_cell_agg = df_cell_agg.sort_values(
            by=["query_cell", "image_count"], ascending=[True, False]
        )
        ...
        for _, item in tqdm(
            cell_totals.iterrows(), total=len(cell_totals), desc=f"Hexagons ({season})"
        ):
            ...
            try:
                ...
                for row in top_clusters:
                    ...
                    desc = (
                        row["cluster_description"] if row["cluster_description"] else ""
                    )
                    parent_label = (
                        row.get("parent_cluster_label", "") if has_parent_col else ""
                    )
                    cluster_list.append(
                        [
                            row["cluster_label"],
        ...
        is_default = (
            (season == "Summer")
            or (len(unique_seasons) == 1)
            or (season == unique_seasons[0] and "Summer" not in unique_seasons)
        )
        layer_name = (
            f"🍂 Season: {season}" if season != "Unknown" else "📍 All Observations"
        )

        fg = folium.FeatureGroup(name=layer_name, show=is_default)

        folium.GeoJson(
            geojson_data, style_function=style_fn, on_each_feature=js_tooltip_builder
        ).add_to(fg)

...
def extract_model_embeddings(model, batch_tensors, representation_type="cls"):
    ...
    if representation_type in ["avg_patch", "cls_avg_patch"]:
        ...
        if (H, W) != (new_H, new_W):
            batch_tensors_resized = F.interpolate(
                batch_tensors, size=(new_H, new_W), mode="bicubic", align_corners=False
            )
        else:
...
def _extract_features_timm(model, batch_tensors):
    ...
    if features.ndim == 4:
        ...
        patch_tokens_vals = (
            features.permute(0, 2, 3, 1).reshape(b, h * w, c).cpu().numpy()
        )
    else:
...
def load_vision_model(model_name, device):
    ...
    try:
        is_timm = model_name in timm.list_models() or any(
            x in model_name
            for x in ["vit_", "resnet", "efficientnet", "convnext", "swin"]
        )
    except Exception:
        pass

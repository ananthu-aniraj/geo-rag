...
"""Text encoder implementation in PyTorch, ported from official TIPS repository."""
...
class MlpBlockWithMask(nn.Module):
    ...
    def __call__(self, inputs: torch.Tensor, mlp_mask: torch.Tensor) -> torch.Tensor:
...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor, mask: torch.Tensor):
        attn_mask = (
            mask[:, None, None, :].repeat(1, self.n_head, x.shape[0], 1).flatten(0, 1)
        )
...
class GlobalAvgPooling(nn.Module):
    ...
    def __init__(self, pooling_dims: t.Sequence[int], epsilon: float = 1e-8):
        super().__init__()
...
class TextEncoder(nn.Module):
    ...
    def __call__(
        ...
        x = self.ln_final(x)
        x = (
            x.pooling(x, compatible_paddings=paddings[:, :, None])
            if hasattr(x, "pooling")
            else self.pooling(x, compatible_paddings=paddings[:, :, None])
        )
        return x

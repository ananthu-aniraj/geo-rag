...
def main():
    ...
    print(
        f"Processing dataset of {num_rows} images in chunks of {chunk_size} with parallel downloads..."
    )
    ...
    for chunk_start in range(0, num_rows, chunk_size):
        ...
        if len(valid_imgs) > 0:
            for b_start in tqdm(
                range(0, len(valid_imgs), args.batch_size), desc="Processing Batches"
            ):
                ...
                try:
                    ...
                    if embeddings_matrix is None:
                        ...
                        dtype = (
                            np.float32 if args.precision == "float32" else np.float16
                        )
                    ...
                    for b_i, global_idx in enumerate(batch_indices):
                        embeddings_matrix[global_idx] = features[b_i].astype(
    ...
    if len(successful_indices) < num_rows:
        print(
            f"Filtering out {num_rows - len(successful_indices)} rows that failed to load or download..."
        )

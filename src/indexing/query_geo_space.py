...
def format_cell_output(df_cell, target_cell, res, total_images):
    ...
    for _, row in df_sorted.iterrows():
        ...
        print(
            f"{row['cluster_label'][:35]:<35} | {pct:>9.2f}% | {row['image_count']:>11}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Query H3 spatial-semantic index for local land-cover and activities."
    )
    parser.add_argument(
        "--lat", type=float, required=True, help="Latitude of the query location."
    )
    parser.add_argument(
        "--lng", type=float, required=True, help="Longitude of the query location."
    )
    parser.add_argument(
        "--res",
        type=int,
    ...
    if not df_cell.empty:
        ...
        df_neighbors = df[
            (df["resolution"] == args.res) & (df["query_cell"].isin(neighbors))
        ]
        ...
        if not df_neighbors.empty:
            ...
            format_cell_output(
                df_agg, f"{target_cell} (Aggregated neighbors)", args.res, total_images
            )
        else:
            print(
                "❌ No observations found in the immediate neighborhood at this resolution."
            )

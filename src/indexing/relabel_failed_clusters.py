...
def load_image(
    url, target_max=448, timeout=15, image_root_dir=None, photo_id=None, platform=None
):
...
def query_vlm_openai_api(
    image_base64, prompt_text, model_name, endpoint_url, timeout=60
):
...
def main():
    ...
    if args.cluster_ids:
        ...
        print(
            f"Forced re-labeling for specific cluster IDs: {sorted(list(target_cluster_ids))}"
        )
    else:
        print(
            f"Automatically detected {len(target_cluster_ids)} failed or unlabeled child clusters."
        )
        if has_parents:
            print(
                f"Automatically detected {len(target_parent_ids)} failed or unlabeled parent clusters."
            )

    if not target_cluster_ids and not target_parent_ids:
        print(
            "No failed or unlabeled child or parent clusters found! Script will exit without modifying the file."
        )
        sys.exit(0)
    ...
    if not server_running:
        ...
        print(
            "Please make sure your VLM server is running before executing this script."
        )
        sys.exit(1)
    ...
    if total_clusters > 0:
        ...
        try:
            for idx, cid in enumerate(sorted_failed_ids):
                print(
                    f"[{idx + 1}/{total_clusters}] Processing Child Cluster #{cid}..."
                )

                task = prepare_cluster_task(cid)
                if not task:
                    print(
                        f"  [FAILED] Child Cluster #{cid}: Failed to download representative images."
                    )
                    continue
                ...
                desc_text = query_vlm_openai_api(
                    task["img_str"], p1_text, args.mllm_model, endpoint, timeout=120
                )
                ...
                if desc_text:
                    ...
                    response_text = query_vlm_openai_api(
                        None,
                        step2_prompt_formatted,
                        args.mllm_model,
                        endpoint,
                        timeout=120,
                    )
                ...
                if response_text:
                    ...
                    print(
                        f"  [FAILED] Child Cluster #{cid}: VLM returned empty response."
                    )
    ...
    if total_parents > 0:
        ...
        try:
            for idx, pid in enumerate(sorted_failed_parent_ids):
                print(
                    f"[{idx + 1}/{total_parents}] Processing Parent Cluster #{pid}..."
                )

                task = prepare_parent_task(pid)
                if not task:
                    print(
                        f"  [FAILED] Parent Cluster #{pid}: Failed to download representative images."
                    )
                    continue
                ...
                if response_text:
                    ...
                    print(
                        f"  [FAILED] Parent Cluster #{pid}: VLM returned empty response."
                    )

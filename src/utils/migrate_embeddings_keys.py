...
def main():
    parser = argparse.ArgumentParser(
        description="Migrate existing decoupled embeddings to stable photo_key maps."
    )
    parser.add_argument(
        "--parquet", type=str, required=True, help="Path to the Parquet metadata file."
    )
    parser.add_argument(
        "--npy", type=str, required=True, help="Path to the .npy embeddings file."
    )
    parser.add_argument(
        "--update_parquet",
        action="store_true",
    ...
    if "Platform" not in df.columns or "Photo_ID" not in df.columns:
        raise ValueError(
            "Metadata Parquet must contain 'Platform' and 'Photo_ID' columns to generate stable keys."
        )
    ...
    if is_aligned_1to1:
        ...
        for idx, key in zip(indices, keys):
            if 0 <= idx < npy_len:
                ...
                print(
                    f"Warning: Index {idx} is out of bounds for npy length {npy_len}. Skipping."
                )
    ...
    if args.update_parquet:
        print(
            f"Appending 'photo_key' column to original Parquet file: {args.parquet}..."
        )

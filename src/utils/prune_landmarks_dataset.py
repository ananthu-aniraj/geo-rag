...
def main():
    parser = argparse.ArgumentParser(
        description="Prune extracted Google Landmarks v2 images to match your sampled CSV."
    )
    parser.add_argument(
        "--csv",
        type=str,
    ...
    for root, dirs, files in tqdm(
        os.walk(args.img_dir, topdown=False), desc="Pruning directories"
    ):
    ...
    if args.dry_run:
        ...
        print(
            f"Scan complete. Would have deleted {deleted_count:,} / {total_scanned:,} scanned images."
        )

...
def main():
    ...
    parser.add_argument("--pre_decoupled", type=str, required=True,
                        help="Path to the uncorrupted pre-decoupled Parquet file containing the 'embedding' column.")
    parser.add_argument("--latest", type=str, required=True,
    ...
    if 'embedding' not in df_pre.columns:
        print("Error: The pre-decoupled file must contain the raw 'embedding' column to preserve alignment.")
        sys.exit(1)

    print(f"Loading license metadata from: {args.latest}...")
    # Load only necessary columns to save memory
    df_latest = pd.read_parquet(args.latest, columns=['Platform', 'Photo_ID', 'License'])
    ...
    print(f" -> Merged successfully. Found {license_count:,} non-null license entries in the merged output.")

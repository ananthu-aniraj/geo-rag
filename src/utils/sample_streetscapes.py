...
def build_city_h3_groups(simplemaps_path, h3_path):
    ...
    h = pd.read_csv(
        h3_path,
        usecols=['uuid', 'h3_8'],
        dtype={'uuid': 'string', 'h3_8': 'category'}
    )

    # Merge on uuid
    merged = pd.merge(m, h, on='uuid')
    print(f"Loaded and merged {len(merged):,} rows in {time.time() - start_time:.2f} seconds.")
    return merged
...
def sample_geographically(df_merged, max_per_city=4000):
    ...
    """
    print(f"Performing geographically stratified sampling (max {max_per_city} per city)...")
    start_time = time.time()
...
def filter_metadata(metadata_path, selected_uuids, output_path, chunk_size=1000000):
    ...
    for chunk in chunks:
        ...
        chunk_idx += 1
        print(f"  Processed {chunk_idx * chunk_size:,} metadata rows... (Written: {total_written:,})")

    print(f"Finished writing in {time.time() - start_time:.2f} seconds. Output contains {total_written:,} records.")
...
def main():
    ...
    parser.add_argument("--max_per_city", type=int, default=4000,
                        help="Maximum images to select per city (default: 4000).")
    parser.add_argument("--out", type=str, default="streetscapes_balanced_subset.csv",

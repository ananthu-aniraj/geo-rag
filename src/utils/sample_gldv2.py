...
def fetch_wikimedia_timestamps(image_titles):
    ...
    batch_size = 50
    print(
        f"Fetching upload timestamps from MediaWiki API for {len(full_titles):,} images in batches of 50..."
    )
    for i in tqdm(range(0, len(full_titles), batch_size), desc="Fetching timestamps"):
                ...
                for single_attempt in range(3):
                    try:
                        res_single = requests.post(
                            url, data=single_params, headers=headers, timeout=10
                        )
                        if res_single.status_code == 200:
                            ...
                            for page_id, page_info in pages_single.items():
                                ...
                                if title and imageinfo:
                                    ts = imageinfo[0].get("timestamp")
                                    normalized_resp_title = title.replace(
                                        "_", " "
                                    ).strip()
                                    orig_title = title_to_orig.get(
                                        normalized_resp_title
                                    )
                                    if orig_title and ts:
                                        timestamps[orig_title] = ts
                            break
        ...
        time.sleep(
            1.0
        )  # Polite delay to respect Wikimedia API limits (approx 60 requests/min max)
...
def main():
    parser = argparse.ArgumentParser(
        description="Sample Google Landmarks v2 dataset in under-represented zones."
    )
    parser.add_argument(
        "--dataset_parquet",
        type=str,
    ...
    parser.add_argument(
        "--cache_path",
        type=str,
        default="category_coords_cache.json",
        help="Path to coordinates JSON cache.",
    )
    parser.add_argument(
        "--output_csv",
        type=str,
        default="google_landmarks_sampled.csv",
    ...
    if args.output_csv and os.path.exists(args.output_csv):
        print(
            f"Found existing output CSV at '{args.output_csv}'. Checking if auto-resume for timestamp fetching is needed..."
        )
        try:
            df_existing = pd.read_csv(args.output_csv, dtype=str)
            if "Captured_At" in df_existing.columns:
                missing_ts = df_existing["Captured_At"].isna() | (
            ...
            if missing_ts.any() and args.fetch_timestamps:
                print(
                    f" -> Found {missing_ts.sum():,} records with missing timestamps. Resuming timestamp fetch..."
                )
                id_to_title = {}
                missing_df = df_existing[
                    missing_ts
                    & df_existing["Image_URL"].notna()
                ...
                if not missing_df.empty:
                    ...
                    if id_to_ts:
                        ...
                        df_existing["Captured_At"] = [
                            ...
                            for pid, current_val in zip(
                                df_existing["Photo_ID"], df_existing["Captured_At"]
                            )
                        ]
                        print(
                            f" -> Successfully resolved upload timestamps for {len(id_to_ts):,} images."
                        )
                        df_existing.to_csv(args.output_csv, index=False)
                        print(
                            f" -> Updated output saved to: {os.path.abspath(args.output_csv)}"
                        )
                        print("Timestamp fetch resume complete!")
                        return
                    else:
                        print(
                            "No timestamps could be resolved. Output remains unchanged."
                        )
                        return
                else:
                ...
                print(
                    "All records already have valid timestamps or '--fetch_timestamps' was not requested. No resume needed."
                )
                print(
                    "If you wish to force a full re-sampling run, please delete the existing CSV file first."
                )
                return
        except Exception as e:
            print(
                f"Warning: Failed to load pre-existing output CSV for resume check: {e}. Re-running full sampling..."
            )
        ...
        if "H3_Cell" in df_existing.columns:
            ...
            for cell in df_existing["H3_Cell"].dropna():
                try:
                    ...
                    target_cell = (
                        h3.cell_to_parent(cell, args.target_h3_res)
                        if res > args.target_h3_res
                        else cell
                    )
            ...
            for lat, lon in zip(
                df_existing["Latitude"].dropna(), df_existing["Longitude"].dropna()
            ):
                try:
    ...
    uncached_cats = [
        c for c in df_cat["clean_name"].dropna().unique() if c not in coords_cache
    ]

    if uncached_cats:
        print(
            f"Querying Wikidata SPARQL endpoint for {len(uncached_cats):,} uncached categories in batches..."
        )
        batch_size = 1000
        for idx in tqdm(
            range(0, len(uncached_cats), batch_size), desc="Wikidata Geocoding"
        ):
    ...
    if args.clean_csv:
        if os.path.exists(args.clean_csv):
            ...
            print(
                f"Warning: Clean CSV '{args.clean_csv}' not found. No clean filter will be applied."
            )
    ...
    for chunk in pd.read_csv(args.landmarks_csv, chunksize=chunksize):
        ...
        # Add coords columns
        valid_chunk["latitude"] = valid_chunk["landmark_id"].map(
            lambda x: landmark_coords[x]["latitude"]
        )
        valid_chunk["longitude"] = valid_chunk["landmark_id"].map(
            lambda x: landmark_coords[x]["longitude"]
        )
        valid_chunk["category_url"] = valid_chunk["landmark_id"].map(
            lambda x: landmark_coords[x]["category_url"]
        )
        ...
        def get_h3(row):
            try:
                return h3.latlng_to_cell(
                    row["latitude"], row["longitude"], args.target_h3_res
                )
            except Exception:
                return None
        ...
        for _, row in valid_chunk.iterrows():
            ...
            if existing_count < args.density_threshold:
                ...
                if (
                    sampled_in_cell < args.max_per_cell
                    and sampled_for_landmark < args.max_per_landmark
                ):
                    # Get cleaned landmark label category name
                    cat_name = "Unknown Landmark"
                    if "clean_name" in df_cat.columns:
                        matches = df_cat[df_cat["landmark_id"] == landmark_id][
                            "clean_name"
                        ].values
                    ...
                    rel_img_path = (
                        f"./images/{img_id[0]}/{img_id[1]}/{img_id[2]}/{img_id}.jpg"
                    )
    ...
    if sampled_records:
        ...
        if args.attribution_csv:
            if os.path.exists(args.attribution_csv):
                print(
                    f"Resolving precise image licenses and direct Wikimedia URLs from '{args.attribution_csv}' using streaming lookup..."
                )
                ...
                try:
                    for chunk in pd.read_csv(
                        ...
                        chunksize=attr_chunksize,
                        usecols=["id", "license", "title"],
                    ):
                ...
                if id_to_license:
                    df_sampled["License"] = df_sampled["Photo_ID"].map(
                        lambda x: id_to_license.get(x, "CC BY-SA 4.0")
                    )
                    print(
                        f" -> Successfully resolved precise licenses for {len(id_to_license):,} images."
                    )
            else:
                print(
                    f"Warning: Attribution CSV '{args.attribution_csv}' not found. Defaulting to 'CC BY-SA 4.0'."
                )

        # Map Wikimedia URLs (will be empty string if not resolved)
        if id_to_wiki_url:
            df_sampled["Image_URL"] = df_sampled["Photo_ID"].map(
                lambda x: id_to_wiki_url.get(x, "")
            )
        ...
        if args.fetch_timestamps and id_to_wiki_url:
            ...
            if id_to_ts:
                df_sampled["Captured_At"] = df_sampled["Photo_ID"].map(
                    lambda x: id_to_ts.get(x, "")
                )
                print(
                    f" -> Successfully resolved upload timestamps for {len(id_to_ts):,} images."
                )
        ...
        print("\nSampling Complete!")
        print(
            f" -> Sampled {len(df_sampled):,} Google Landmarks images in under-represented zones."
        )
        if "h3_cell" in df_sampled.columns:
            print(
                f" -> Cover {df_sampled['h3_cell'].nunique():,} unique under-represented H3 cells."
            )

        # Drop columns not needed in final metadata CSV (Image_Location, h3_cell)
        cols_to_drop = [
            c for c in ["Image_Location", "h3_cell"] if c in df_sampled.columns
        ]
        if cols_to_drop:

...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Number of database images to search against.",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
    ...
    parser.add_argument(
        "--output_report",
        type=str,
        default="./benchmark_results/env_zones_report.txt",
        help="Path to write the report summary.",
    )
    parser.add_argument(
        "--output_csv",
        type=str,
        default="./benchmark_results/env_zones_results.csv",
    ...
    url_col = next(
        (col for col in df.columns if col.lower() in ["image_url", "url"]), None
    )
    id_col = next(
        (col for col in df.columns if col.lower() in ["photo_id", "id"]), None
    )
    ...
    continent_col = next(
        (col for col in df.columns if col.lower() == "continent"), None
    )
    if continent_col:
        print("Filtering European coordinates using the CSV 'Continent' column...")
        df = df[df[continent_col].astype(str).str.lower() == "europe"].reset_index(
            drop=True
        )
        print(f"Keep {len(df)} records inside Europe.")
    else:
        print(
            "Warning: 'Continent' column not found in database. Proceeding without European filtering."
        )
    ...
    print(
        "Partitioning queries and database geographically to prevent spatial autocorrelation/sequence leakage..."
    )
    ...
    query_pool = []
    class_q_lists = {
        cls: [item for item in query_by_class.get(cls, []) if matches_query_plat(item)]
        for cls in sorted_classes
    }
    ...
    max_db_len = (
        max(len(lst) for lst in class_db_lists.values()) if class_db_lists else 0
    )
    ...
    print(
        f"Downloading {len(queries_selection)} query images for benchmarking in parallel..."
    )
    queries_dict = {}
    ...
    with ThreadPoolExecutor(max_workers=32) as executor:
        ...
        for future in tqdm(
            as_completed(futures), total=len(futures), desc="Downloading Queries"
        ):
            idx = futures[future]
            img = future.result()
            if img:
    ...
    print(
        f"Final evaluation split: {len(queries_meta)} Queries, {len(database_meta)} Database (processing DB in chunks)."
    )
        ...
        for i in tqdm(
            ...
            for idx in range(len(batch_imgs)):
                ...
                if is_local:
                    ...
                    representations[f"{model_label} 1st CLS + Average Patch"][
                        split_key
                    ].append(np.concatenate([first_cls_token, avg_patch]))
                    representations[f"{model_label} 2nd CLS + Average Patch"][
                        split_key
                    ].append(np.concatenate([second_cls_token, avg_patch]))
                else:
    ...
    print(
        f"Processing database of {len(database_meta)} images in chunks of {db_chunk_size}..."
    )
    for chunk_start in range(0, len(database_meta), db_chunk_size):
        ...
        with ThreadPoolExecutor(max_workers=32) as executor:
            ...
            for future in tqdm(
                as_completed(futures), total=len(futures), desc="Downloading DB Chunk"
            ):
                idx = futures[future]
                img = future.result()
                if img:
    ...
    report_lines = []
    report_lines.append(
        "Environmental Zones of Europe Image Representation Semantic Retrieval Report"
    )
    ...
    if args.output_html and rep_viz_records:
        ...
        for rep_name, records in rep_viz_records.items():
            ...
            rep_metrics = results.get(rep_name, {})

            title = f"Environmental Zones Retrieval Visualizer ({rep_name})"

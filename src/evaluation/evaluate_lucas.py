...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate local VLMs against LUCAS 2018 using Ollama or sglang."
    )
    parser.add_argument(
        "--backend",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="v1",
        help="The version of the prompt to use (folder name in prompts_lucas/).",
    )
    parser.add_argument(
        "--retrieval_caption_mode",
        type=str,
        default="natural",
        choices=[
            "natural",
        ...
        help="Mode for constructing the caption used in retrieval evaluation.",
    )
    ...
    print(
        f"Selected {len(selected_images)} images representing {len(set(img['point_id'] for img in selected_images))} unique locations."
    )
    ...
    for item in selected_images:
        ...
        try:
            ...
            # If this is v3 or v4 (having structured keys), map them to standard keys for database & retrieval compatibility
            if "dominant_cover" in parsed_data1:
                ...
                human_activities_struc = (
                    f"Human evidence: {parsed_data1.get('human_evidence', '')}."
                )
                ...
                if "human_activities" not in parsed_data1:
                    # v4: Only visible_evidence is generated by VLM
                    visible_evidence = parsed_data1.get("visible_evidence", "")
                    human_activities = ""
                    land_cover_usage = ""
                    type_of_vegetation = ""
                else:
                    # v3: All four natural language fields are present
                    has_natural = "visible_evidence" in parsed_data1 and not str(
                        parsed_data1.get("visible_evidence", "")
                    ).startswith("Dominant cover is")
                    visible_evidence = (
                        parsed_data1.get("visible_evidence", visible_evidence_struc)
                        if has_natural
                        else visible_evidence_struc
                    )
                    human_activities = (
                        parsed_data1.get("human_activities", human_activities_struc)
                        if has_natural
                        else human_activities_struc
                    )
                    land_cover_usage = (
                        parsed_data1.get("land_cover_usage", land_cover_usage_struc)
                        if has_natural
                        else land_cover_usage_struc
                    )
                    type_of_vegetation = (
                        parsed_data1.get("type_of_vegetation", type_of_vegetation_struc)
                        if has_natural
                        else type_of_vegetation_struc
                    )
            ...
            if "{dominant_cover}" in step2_prompt:
                step2_prompt = step2_prompt.replace(
                    "{dominant_cover}", str(parsed_data1.get("dominant_cover", ""))
                )

            # Step 2 always uses the structured variables to maximize classification performance
            step2_prompt = (
                step2_prompt.replace("{visible_evidence}", str(visible_evidence_struc))
            ...
            # Construct caption based on the selected mode for ablation testing
            if args.retrieval_caption_mode == "natural":
                combined_caption = visible_evidence
            elif args.retrieval_caption_mode == "natural_plus_human":
                combined_caption = (
                    f"{visible_evidence} {human_activities_struc}"
                    if human_activities_struc
                    else visible_evidence
                )
            elif args.retrieval_caption_mode == "natural_plus_veg":
                combined_caption = (
                    f"{visible_evidence} {type_of_vegetation_struc}"
                    if type_of_vegetation_struc
                    else visible_evidence
                )
            elif args.retrieval_caption_mode == "structured_only":
                combined_caption = f"{visible_evidence_struc} {human_activities_struc} {land_cover_usage_struc} {type_of_vegetation_struc}"
            elif args.retrieval_caption_mode == "all_v3":
                caption_parts = [
                    visible_evidence,
                    human_activities,
                    land_cover_usage,
                ...
                combined_caption = " ".join(
                    [str(p).strip() for p in caption_parts if p and str(p).strip()]
                )

            # Clean up double dots or extra spaces
            combined_caption = " ".join(combined_caption.split()).replace("..", ".")
            ...
            # Exact match check
            exact_lc = (
                str(pred_lc).strip().lower() == str(meta["lc_label"]).strip().lower()
            )
            exact_lu = (
                str(pred_lu).strip().lower() == str(meta["lu_label"]).strip().lower()
            )
            exact_eunis = (
                str(pred_eunis).strip().lower()
                == str(meta["eunis_class"]).strip().lower()
            )

            print(
                f"  -> Land Cover Sim: {sim_lc:.4f} (Exact: {exact_lc}) | Pred: {pred_lc}"
            )
            print(
                f"  -> Land Use Sim: {sim_lu:.4f} (Exact: {exact_lu}) | Pred: {pred_lu}"
            )
            print(
                f"  -> EUNIS Sim: {sim_eunis:.4f} (Exact: {exact_eunis}) | Pred: {pred_eunis}"
            )
            print(
                f"  -> CLIP Sim: {clip_similarity:.4f} | TIPSv2 Sim: {tips_similarity:.4f}"
            )
            ...
            # Save all raw Step 1 keys into the pickle for retrieval evaluation
            for k, v in parsed_data1.items():
    ...
    if valid_evaluations > 0:
        ...
        summary_report.append(
            f"Average CLIP Similarity: {results_df['clip_similarity'].mean():.4f}"
        )
        summary_report.append(
            f"Average TIPSv2 Similarity: {results_df['tips_similarity'].mean():.4f}"
        )

        # Class-level scores
        summary_report.append("\n--- Metric Summaries ---")
        summary_report.append(
            f"Land Cover (lc_label) Semantic Sim: {results_df['sim_lc'].mean():.4f}"
        )
        summary_report.append(
            f"Land Cover (lc_label) Exact Match: {(results_df['exact_lc'].sum() / len(results_df)) * 100:.2f}%"
        )

        summary_report.append(
            f"Land Use (lu_label) Semantic Sim: {results_df['sim_lu'].mean():.4f}"
        )
        summary_report.append(
            f"Land Use (lu_label) Exact Match: {(results_df['exact_lu'].sum() / len(results_df)) * 100:.2f}%"
        )

        summary_report.append(
            f"EUNIS Class Semantic Sim: {results_df['sim_eunis'].mean():.4f}"
        )
        summary_report.append(
            f"EUNIS Class Exact Match: {(results_df['exact_eunis'].sum() / len(results_df)) * 100:.2f}%"
        )
        ...
        for direction_val, group in results_df.groupby("direction"):
            ...
            summary_report.append(
                f"  Land Cover Sim: {group['sim_lc'].mean():.4f} (Exact: {(group['exact_lc'].sum() / len(group)) * 100:.2f}%)"
            )
            summary_report.append(
                f"  Land Use Sim:   {group['sim_lu'].mean():.4f} (Exact: {(group['exact_lu'].sum() / len(group)) * 100:.2f}%)"
            )
            summary_report.append(
                f"  EUNIS Class Sim: {group['sim_eunis'].mean():.4f} (Exact: {(group['exact_eunis'].sum() / len(group)) * 100:.2f}%)"
            )
            summary_report.append(
                f"  CLIP Similarity: {group['clip_similarity'].mean():.4f}"
            )
        ...
        summary_file = (
            f"vlm_lucas_evaluation_summary_{model_tag}_{args.prompt_version}.txt"
        )
        ...
        output_csv = (
            f"vlm_lucas_evaluation_results_{model_tag}_{args.prompt_version}.csv"
        )
        ...
        retrieval_file = (
            f"vlm_lucas_retrieval_data_{model_tag}_{args.prompt_version}.pkl"
        )

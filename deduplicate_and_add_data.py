...
def main():
    ...
    parser.add_argument("--existing_csv", type=str, required=True,
                        help="Path to the current consolidated CSV file containing already processed data.")
    parser.add_argument("--new_data_dirs", type=str, nargs="+", required=True,
    ...
    if os.path.exists(args.existing_csv):
        ...
        print(f"Existing CSV not found at '{args.existing_csv}'. Proceeding with empty duplicate database.")

    # 2. Gather new metadata CSVs from the specified directories
    new_csv_files = []
    for d in args.new_data_dirs:
        if os.path.isdir(d):
            found_csvs = glob.glob(os.path.join(d, "**", "*.csv"), recursive=True) + glob.glob(os.path.join(d, "*.csv"))
    ...
    new_csv_files = [os.path.abspath(f) for f in new_csv_files if os.path.abspath(f) != existing_abs]

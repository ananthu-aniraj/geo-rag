...
def main():
    ...
    parser.add_argument("--csv_path", type=str, default=DEFAULT_CSV_PATH, help="Path to the iWildCam subset metadata CSV.")
    parser.add_argument("--images_dir", type=str, default=DEFAULT_IMAGES_DIR, help="Directory containing iWildCam images.")
    parser.add_argument("--output_dir", type=str, default=DEFAULT_OUTPUT_DIR, help="Base directory to save results.")
    parser.add_argument("--query_idx", type=int, default=8, help="Row index of the query image in the subset.")
    ...
    cache_path = os.path.join(os.path.dirname(args.csv_path), "iwildcam_embeddings_cache.pkl")
    import pickle
    ...
    if os.path.exists(cache_path):
        ...
        for idx, row in tqdm(df.iterrows(), total=len(df), desc="Inference"):
            ...
            for r in range(32):
                for c in range(32):
                    patch_pixels = pred_segmentation[r * 14:(r + 1) * 14, c * 14:(c + 1) * 14]
    ...
    def get_diagnostic_image(idx):
        ...
        binary_mask_visual = (np.array(img_resized) * 0.6 + mask_overlay * 0.4).astype(np.uint8)
    ...
    for idx in valid_indices:
        if idx == args.query_idx: continue
    ...
    def print_matches_and_eval(name, top_matches, save_dir, save_masks=False):
        ...
        for rank, (idx, sim) in enumerate(top_matches, 1):
            row = df.iloc[idx]
            is_correct = "CORRECT (Same Camera)" if row['location'] == query_location else "INCORRECT (Diff Camera)"
            ...
            if save_masks:
                match_diag_path = os.path.join(save_dir, f"match_{rank}_sim_{sim:.4f}_segmentation.png")
    ...
    # Print and Evaluate
    p3_cls = print_matches_and_eval("STANDARD [CLS]", top_cls, cls_dir, save_masks=False)
    p3_simple = print_matches_and_eval("[SIMPLE-AVERAGE]", top_simple, simple_dir, save_masks=False)
    p3_bg = print_matches_and_eval("[BACKGROUND-AVERAGE]", top_bg, bg_dir, save_masks=True)
    p3_concat_simple = print_matches_and_eval("[CLS + SIMPLE-AVG CONCAT]", top_concat_simple, concat_simple_dir, save_masks=False)
    p3_concat_bg = print_matches_and_eval("[CLS + BG-AVG CONCAT]", top_concat_bg, concat_bg_dir, save_masks=False)
